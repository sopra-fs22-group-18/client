...
const ActiveSession = () => {
  ...
  function MessageAdd(message) {
    setInputMessages((messages) => [
      ...messages,
      <div className="chatMessage" key={messageIndex}>
        {" "}
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error(
          `Something went wrong while getting the data for the session: \n${handleError(
            error
          )}`
        );
        console.error("Details:", error);
        alert(
          "Something went wrong while getting the data for the session! See the console for details."
        );
      }
    };
  ...
  const leaveSession = async () => {
    try {
      ...
      alert(
        `Something went wrong when trying to leave the session: \n${handleError(
          error
        )}`
      );
    }
  ...
  let reportComments = (
    <Button width="100%" onClick={() => reportComment()}>
      {" "}
      Report comment
    </Button>
  );
  ...
  let leaveSessionButton = (
    <Button5 width="100%" onClick={() => leaveSession()}>
      {" "}
      <div className="leaveSession">
        <div width="90%">Leave session</div>{" "}
        <div width="10%">
  ...
  let ShowMessage = (
    <div>
      <h1>The Winner is: {sessionWinner}</h1>
    </div>
  );
  ...
  let hideParticipants = (
    <div>
      <Button4 width="100%" onClick={() => hideAllParticipants()}>
  ...
  let showParticipants = (
    <div>
      <Button4 width="100%" onClick={() => selectTheWinner()}>
        Select the Winner
      </Button4>
    </div>
  );
        ...
        <div class="session rightChild">
          ...
          <div className="newComment">
            <div className="newComment container">
              ...
              <div>&nbsp;</div>
              <div className="reportcomment">{reportComments}</div>
            </div>
          </div>
        </div>
      </div>
